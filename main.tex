...
way that $E = \overrightarrow E + \overleftarrow E$. Let $L(G)$ be the
...
is a symmetric probability with margin $\rho$, so that
$\frac{\theta(x,y)}{\rho(x)} = \theta(y | x)$, e.g. the conditional.

\section{Arens-Eells norm}
