...
\address{Collegio Carlo Alberto}
...
$\theta(x,y) = 0$ if $\omega(x,y) = 0$, we define the  inner product
for $a,b \in M(G)$ by
\begin{multline*}
  \scalarat \theta a b = \frac 12 \sum _ {x,y \in V} \theta(x,y)
  a(x,y) b(x,y) = \frac 12 \sum _ {xy \in E} \theta(x,y) a(x,y)
  b(x,y) = \\ \sum_{xy \in \overrightarrow E} \theta(x,y)
  \frac12(a(x,y)b(x,y)+a(y,x)b(y,x)) \ . 
\end{multline*}

If $a$ and $b$ are either both symmetric, or both skew-symmetric, then
\begin{equation*}
  \scalarat \theta a b = \sum_{xy \in \overrightarrow E} \theta(x,y)
  a(x,y)b(x,y) \ .
\end{equation*}
